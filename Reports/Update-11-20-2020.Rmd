...
df <- read.table(file = file.path(path,"R_Input_2.txt"), 
                 ...
                 sep = "\t", 
                 row.names = 6)
tb <- read_tsv(file = file.path(path,"R_Input_2.txt"))
...
tb.tp <- read_tsv(file = file.path(path,"Tumor_purity.txt"))

df_Cancer <- read.table(file = file.path(path,"Cancer_type.txt"), header = T, sep = "\t")
row.names(df_Cancer) <- df_Cancer$Sample_ID

tb.ca <- read_tsv(file = file.path(path,"Cancer_type.txt"))

# merge tibbles using reduce
tables <- list(tb,tb.tp,tb.ca)
tb_1 <- tables %>% reduce(full_join) 
...
for(j in fix_effects){
  for(i in metrics) {
   ...
   # compare means between MMRd groups, grouped by cancer types
    corr_tmp <- compare_means(as.formula(paste0(i," ~ ",j)), data = df_2[!df_2$MMRd_Type %in% c("?","Delete","NORMAL"),], group.by = "Cancer_Type_NEW")
    ...
    assign(paste0("p_",i), plotCorr.MMRd(df_2_pls_1[!df_2_pls_1$MMRd_Type %in% c("?","Delete","NORMAL"),],i,j))
    ...
    #re <- c(k,"Sample_Type")
    re <- c("Sample_Type")
    fits <- buildLmeModel(i,fe,re,df_2[!df_2$MMRd_Type %in% c("?","Delete","NORMAL"),])
      # make HTML report
...
df_surv_MMRd.CancerType.GeneMut.collapsedType <- df_surv_MMRd.CancerType.GeneMut.collapsedType[
  !is.na(df_surv_MMRd.CancerType.GeneMut.collapsedType$Overall_Survival_Months) & !df_surv_MMRd.CancerType.GeneMut.collapsedType$CLASSIFICATION_PLOT %in% c("Delete","NORMAL","?"),
...
# merge with df_2 to include genomic features to df_surv
df_surv_grand_total <- merge(df_surv_MMRd.CancerType.GeneMut.collapsedType, df_2, 
                             by = 0, no.dups = TRUE)
row.names(df_surv_grand_total) <- df_surv_grand_total$Row.names
df_surv_grand_total[,"status"] <- as.numeric(as.factor(df_surv_grand_total[,"status"]))
...
#groups <- c("MMRd","Multiplex_or_not", "MSH6_or_not", "PMS2_or_not")
groups <- c("MMRd","Multiplex_or_not", "MSH6_or_not", "PMS2_or_not")
...
# univariate cox ph modeling
covariates <- c("MMRd","Cancer_Type_NEW","KRAS_Mut","Multiplex_or_not.x",
                "MSH6_or_not.x","MSI_Score_1","Impact_TMB_Score","No_Missense",
                "No_Frameshift","Tumor_Purity","Sample_Type","Current_Age")

univ_formulas <- sapply(covariates,
                        function(x) as.formula(paste('Surv(time, status)~', x)))
                        
univ_models <- lapply( univ_formulas, function(x){coxph(x, data = df_surv_grand_total)})
# Extract data 
...
# Multi-variates coxph model
res.cox <- coxph(Surv(time, status) ~ MMRd + Cancer_Type_NEW + Sample_Type + KRAS_Mut, data =  df_surv_grand_total)
summary(res.cox)
...
ggplot(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),], aes(x = MMRd_Type, y = log10(Aneuploidy.Score))) +
  geom_boxplot()

ggplot(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),], aes(x = Multiplex_or_not, y = log10(Aneuploidy.Score))) +
  geom_boxplot()

ggplot(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),], aes(x = MSH6_or_not, y = log10(Aneuploidy.Score))) +
  geom_boxplot()

ggplot(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),], aes(x = PMS2_or_not, y = log10(Aneuploidy.Score))) +
  geom_boxplot()

# p_aneuScore.MMRd <- plotCorr.MMRd(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),],"log.Aneuploidy.Score","MMRd_Type")
# p_aneuScore.MMRd
#
# p_aneuScore.Cancer <- plotCorr.Cancer(df_aneuScore_MMR[! df_aneuScore_MMR$MMRd_Type %in% c("?","Delete","NORMAL"),],"log.Aneuploidy.Score")
# p_aneuScore.Cancer
```
...
df_MMRd <- read.table(file = file.path(path,"MMRd_type.txt"), header = T, sep = "\t")
...
median_mut <- log2(median_mut[,!names(median_mut) %in% c("MMRd_type")]/30)
...
mean_mut <- log2(mean_mut[,!names(mean_mut) %in% c("MMRd_type")]/30)
...
#colnames(median_mut) <- paste0(names(median_mut),".median")
# normalize by the size of MSK-IMPACT-468genes size 1.5Mb
# normalize by the size of human exome 30Mb
median_mut <- log2(median_mut[,!names(median_mut) %in% c("MMRd_type")]/30)
median_mut$MMRd_type <- row.names(median_mut)
