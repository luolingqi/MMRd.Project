---
title: "MMRd Project Update, tibble version"
author: "Lingqi Luo"
date: "12/29/2020"
...
# Add the collapsed columns "Multiplex_or_not", "MSH6_or_not", "PMS2_or_not" and "Cancer_Type"
tb_2 <- tb_1 %>% mutate(Multiplex_or_not =
                        ifelse(tb_1$CLASSIFICATION_PLOT %in% c("MSH2_MSH6","MLH1_PMS2","Complex"),
                               "Multiplex", 
                               ifelse(tb_1$CLASSIFICATION_PLOT %in% c("MSH6","PMS2"),
                                      "Singleton", 
                                      tb_1$CLASSIFICATION_PLOT
                                      )
                              )
                        ) %>% mutate(Multiplex_or_not = 
                                       relevel(as.factor(Multiplex_or_not), ref = "Singleton")
                                     ) %>% 
  
                  mutate(MSH6_or_not = 
                        ifelse(str_detect(tb_1$CLASSIFICATION_PLOT,"MSH6"), 
                               #grepl("MSH6", tb_1$CLASSIFICATION_PLOT),
                               "MSH6",
                               "Not_MSH6"
                               )
                         ) %>% mutate(MSH6_or_not = 
                                        relevel(as.factor(MSH6_or_not), ref = "MSH6")
                                      ) %>%
  
                  mutate(PMS2_or_not = 
                         ifelse(str_detect(tb_1$CLASSIFICATION_PLOT,"PMS2"),
                           #grepl("PMS2",tb_1$CLASSIFICATION_PLOT),
                                "PMS2",
                                "Not_PMS2"
                                )
                         ) %>% mutate(PMS2_or_not = 
                                        relevel(as.factor(PMS2_or_not), ref = "PMS2")
                                      )

tb_1 %<>% rename(MMRd_Type = CLASSIFICATION_PLOT) %>% filter(MMRd_Type != "Delete")
tb_2 %<>% rename(MMRd_Type = CLASSIFICATION_PLOT) %>% filter(MMRd_Type != "Delete") %>%
  mutate(Cancer_Type_NEW = relevel(as.factor(Cancer_Type_NEW), ref = "Colon")
         )
...
label(tb_1$Current_Age) <- "Age"
units(tb_1$Current_Age) <- "years"

label(tb_2$Current_Age) <- "Age"
units(tb_2$Current_Age) <- "years"

label(tb_1$Tumor_Purity) <- "Tumor Purity"
units(tb_1$Tumor_Purity) <- "%"

label(tb_2$Tumor_Purity) <- "Tumor Purity"
units(tb_2$Tumor_Purity) <- "%"

label(tb_1$Cancer_Type_NEW) <- "Cancer Type"
label(tb_2$Cancer_Type_NEW) <- "Cancer Type"

label(tb_1$Sample_Type) <- "Sample Type"
label(tb_2$Sample_Type) <- "Sample Type"
```
...
###################################################################
table1(~ Current_Age + Race + Ethnicity + Tumor_Purity + Cancer_Type_NEW + Sample_Type | MMRd_Type, data=tb_1, overall="Total")

```
:::
:::{}
```{r results='hide', error = FALSE, warning = FALSE, message = FALSE, figures-side, fig.show="hold", out.width="50%"}
plotCaseFreq(tb_1, "Cancer_Type_NEW", "MMRd_Type")
```
:::
::::
